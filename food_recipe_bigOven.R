...
df_x <- df_l[, 10:ncol(df_l)]
ingredient <- names(df_x)
vert_raw <- data.frame(ids = 1:length(ingredient), ingredient = ingredient)
links_l <- list()
# construct links in list
for(r in 1:nrow(df_x)){
  linked_node <- which(df_x[r,] != 0 | is.na(df_x[r,])) # all ingredients in same recipe
  if(length(linked_node) > 1){
    all_cb <- combn(length(linked_node), 2) # generate all combinations
    for(c in 1:ncol(all_cb)){
      # assign a link to each combination
      tmp_l = list(id1 = linked_node[all_cb[1,c]],
                   id2 = linked_node[all_cb[2,c]],
                   weight = 1)
      links_l[[length(links_l)+1]] <- tmp_l
    }
  }
}
# convert links list to data frame 
links_raw <- data.frame(matrix(ncol = 3, nrow = length(links_l)))
colnames(links_raw) <- c('id1','id2','weight')
for(i in 1:length(links_l)){
  # links_raw[i, 1] = links_l[[i]]$id1
  # links_raw[i, 2] = links_l[[i]]$id2
  # links_raw[i, 3] = links_l[[i]]$weight
  links_raw[i, ] = links_l[[i]] %>% unlist()
}
links_before_aggregate <- links_raw
# aggregate weights
links_raw <- links_raw %>% 
              group_by(id1, id2) %>% 
...
net <- simplify(net, remove.multiple = F, remove.loops = T) # simplify network
#
plot_net <- function(net, v_size = .1, mar = -.7, label= NA ,
                     edge = 0, title = NULL,e_size=.2, show_label =0){
  # plot
  ...
  E(net)$width <- E(net)$weight*e_size
  #change arrow size and edge color:
  # E(net)$arrow.size <- .2
  # E(net)$edge.color <- "gray80"
  # E(net)$width <- 1+E(net)$weight/12
  if(edge == 0 & show_label == 0){
    plot(net, margin = mar, main = title) 
  }else{
    # edge
    edge.start <- ends(net, es=E(net), names=F)[,1]
    edge.col <- V(net)$color[edge.start]
    plot(net, margin = mar, edge.color=edge.col, edge.curved=.1,
...
# delete edges
s <- 1:nrow(links_raw)
net <- delete.edges(net, s[links_raw$weight < 30])
# plot
...
png(filename = 'network_3.png',  width = 16, height = 9, units = 'in', res = 300)
plot_net(net, v_size =.19, mar= -.2, edge = 1, e_size =.03, show_label = 1)
dev.off()
...
# 5--------------------------
# LASSO: analyze rating
install.packages('glmnet')
library(glmnet)

#delete cluster & names column
df_rating <- df_l[, 3:ncol(df_l)] 
...
# handle NAs and recipes with few ingredients #8
clean_data <- function(df_rcp){ 
  # handel NAs
  for(i in 8:ncol(df_rcp)){
    df_rcp[ ,i] <- as.numeric(df_rcp[,i])
...
df_rating <- clean_data(df_rating)

# handle infinite values
for(c in 2:ncol(df_rating)){
  df_rating[is.infinite(df_rating[, c]), c] <- unique(df_rating[, c]) %>% median( na.rm = T)
  df_rating[is.na(df_rating[,c]), c] <- df_rating[!is.na(df_rating[,c]), c] %>% unique() %>% median()
}
...
#
data_m <- df_rating
x=model.matrix(rating~.,data_m)[,-1] # get rid of the intercept
...
# build lasso model
al=1 # 1 for lasso, 0 for ridge
model_lasso=glmnet(train_x,
                   train_y,
                   alpha=al)
# plot
png(filename = 'lasso_1.png', width = 9, height = 6, units = 'in', res = 150)
...
cv_error=cv.glmnet(train_x,
                   ...
                   alpha=al)
# plot
png(filename = 'lasso_2.png', width = 9, height = 6, units = 'in', res = 150)
plot(cv_error)
dev.off()
...
# split data
training_data = df_rating[train, ]
testing_data = df_rating[-train, ]

model_rf <- randomForest(rating~., training_data,
                         importance =T,
                         proximity=TRUE)
# Variable Importance Plot
png(filename = 'rf_1.png', width = 9, height = 6, units = 'in', res = 150)
varImpPlot(model_rf,
           sort = T,
...
MSE_rf <- mean((pred_rf - testing_data$rating)^2)
